...
const RideDetailsByDriver = () => {
  ...
  useEffect(() => {
    const fetchRideDetails = async () => {
      try {
        const res = await axiosSecureInstance.get(`api/driver/ride-details/${id}`);
        if (!res.data) {
          setNotFound(true);
        } else {
          setRide(res.data);
  ...
  if (loading) return <p className="mt-10 text-center">Loading ride details...</p>;
  if (notFound) return <p className="mt-10 text-center text-red-500">No ride found for this ID</p>;
  ...
  return (
    <div className="max-w-5xl p-4 mx-auto space-y-6">
      ...
      <div className="p-4 space-y-1 bg-white rounded-lg shadow">
        <p><strong>Booking ID:</strong> {ride.bookingID}</p>
        <p><strong>Status:</strong> {ride.status}</p>
        <p><strong>Date & Time:</strong> {ride.date} at {ride.time}</p>
      ...
      <div className="p-4 space-y-4 bg-white rounded-lg shadow">
        ...
        <div>
          ...
          <p><strong>Name:</strong> {ride.contactInfo?.name}</p>
          <p><strong>Email:</strong> {ride.contactInfo?.email}</p>
          <p><strong>Phone:</strong> {ride.contactInfo?.phone}</p>
        </div>

        {/* Vehicle */}
        <div>
          <h2 className="text-lg font-semibold">Vehicle Info</h2>
          <p><strong>Type:</strong> {ride.selectedCar?.vehicleType}</p>
          <p><strong>Model:</strong> {ride.selectedCar?.vehicleModel}</p>
          <p><strong>Year:</strong> {ride.selectedCar?.vehicleYear}</p>
        ...
        <div>
          ...
          <p><strong>Status:</strong> {ride.paymentStatus}</p>
          <p><strong>Amount:</strong> ${ride.totalPrice}</p>
        </div>
      </div>
    </div>
  );

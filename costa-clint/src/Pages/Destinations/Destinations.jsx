...
const Destinations = () => {
  ...
  const translatedDestinations = t("destinations_section.list", { returnObjects: true });
    ...
    <section className="min-h-screen px-6 py-16 my-10 text-gray-900 bg-white">
      <div className="max-w-6xl mx-auto">
        <h2 className="mb-12 text-4xl font-bold text-center">
          {t("destinations_section.title")}
        </h2>
        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3">
          {translatedDestinations.map((d, i) => (
            <div key={i} className="overflow-hidden shadow-md rounded-xl">
              <img
                src={destinationImages[i].image}
                alt={d.name}
              ...
              <div className="p-4 bg-gray-100">
                ...
                <p className="text-sm text-gray-600">
                  {d.airport_label}: {destinationImages[i].airport}
                </p>
              </div>
            </div>
          ))}
